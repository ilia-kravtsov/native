...
Функция принимает параметры, чтобы получать разные результаты от одних и тех же действий 
...
мы воспринимаем вызов функии sum(2,3) как появление того значения которое 
эта функция return
...
_____________________________________________________ day 2

export const addSkill = (st: StudType, skill: string) => {
    st.technologies.push({id: new Date().getTime(), tecName: skill})
}

Мы создали функцию которая добавляет в массив technologies новый объект и затипизировали для 
нее значения которые будут в нее приходить 

Теперь в тесте проверим работоспособность функции попробовав дать функции при ее вызове
конкретные значения 

Перед каждым тестом приравнивай объект stud к 
...
Наша функция не имеет return значит она возвращает undefined
Наша функция addSkill модифицирует те данные которые в нее приходят 
То есть addSkill просто меняет сам объект, она не должна ничего возвращать
так как ее цель в измении свойств объекта а не в возвращении какого-либо значения
как у функции sum 

ООП программирование как раз строится на таких функциях где функции непосредственно
импользуются для изменения поступающих в нее объектов (данных)
Пример - наша функция addSkill

export const addSkill = (st: StudType, skill: string) => {
    st.technologies.push({id: new Date().getTime(), tecName: skill})
}

Функциональное программирование говорит что функции должны быть чистыми 
то есть функция не имеет право менять данные которые в нее приходят
то есть такая функция будет работать иммьютабельно и не будет изменять
изначальные данные но будет возвращать на основе старых данных - новые
Пример - функция sum
...
React Redux базируется на подходе чистых функций функционального прогарммирования 

Тест

Указываем данные с которыми нужно будет работать, в вызове функции beforeEach(()=>{})
которая принимает их в виде callback
указываем типизацию для данных let ctudent: StudType

Пишем test('указываем название теста'), () => { - указываем callback где опишем сам тест
указываем действия над данными в beforeEach результат которых нужно будет сверять со своими ожиданиями
от этих изменений

Пишем что ожидаем после действий выше

expect(обращаемся к конкретному свойству объекта из beforeach).toBe(пишем что ожидаем получить)
expect(ctudent.technologies[4].id).toBeDefined() - просто определяем что id существует 
}
...
параметр st - может быть назван как угодно
параметр и переменная - практически одно и тоже только параметр можно рассматривать как переменную
только в рамках той самой функции в которой он употребляется 

Когда мы создали переменную ctudent в 03.test мы создали ссылку на объект под именем ctudent
когда мы передаем данную ссылку на объект в функцию addSkill в тесте
мы отправляем этот же объект в функцию addSkill только конкретно в этой функции
уже обращаемся к этому же объекту уже через другое имя st которое соответствует той же типизации
что и ctudent StudType

То есть мы работаем с ссылочным типом объектов 

Декларация функции (Function Declaration) создается до выполнения любого кода, 
в то время как функциональное выражение (Function Expression) будет создано только в момент,
...
Чистая функция не имеет право что-либо менять, она обязано что либо вернуть

Сигнатура функции это ее интерфейс то как мы с функцией взаимодействуем
Сигнатура функции это то что функция принимает и что она возвращает

Так как в функциональном программировании функции не имеют права менять объекты 
